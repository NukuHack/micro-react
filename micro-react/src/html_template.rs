...
use crate::bindings::{
    children_to_js, js_ref_to_node_ref, js_to_vnode, js_val_to_prop_val, props_to_js_object,
    vnode_to_js,
};
...
fn build_sentinel_html(statics: &[String]) -> String {
    ...
    for i in 0..n_holes {
        ...
        let looks_like_open = !looks_like_close
            && prev_trimmed.ends_with('<')
            && matches!(next_first, Some(c) if c.is_whitespace() || c == '>' || c == '/');
...
fn compile_node(node: &Node) -> Option<ChildTemplate> {
    match node.node_type() {
        ...
        Node::ELEMENT_NODE => {
            ...
            Some(ChildTemplate::Element(Box::new(ElementTemplate {
                tag,
                static_attrs,
                attr_holes,
                key,
...
fn compile_template(statics: &[String]) -> Result<CompiledTemplate, JsValue> {
    ...
    let doc = parser.parse_from_string(
        &format!("<root>{wrap_prefix}{html}{wrap_suffix}</root>"),
    ...
    for _ in 0..extra_depth {
        root = root
            .first_child()
            .ok_or("html`: DomParser produced unexpected table structure")?;
    }
...
thread_local! {
    ...
    static TEMPLATES: RefCell<Vec<CompiledTemplate>> = RefCell::new(Vec::new());
}
...
fn render_element(tpl: &ElementTemplate, values: &Array) -> Option<VNode> {
    ...
    let node_ref: Option<NodeRef> = tpl
        .ref_hole
        .and_then(|i| js_ref_to_node_ref(&values.get(i as u32)));
...
mod pure_logic_tests {
    ...
    fn self_closing_non_void_element_gets_explicit_close_tag() {
        assert_eq!(
            expand_self_closing_tags(r#"<div class="x" />"#),
            r#"<div class="x"></div>"#
        );
    }
    ...
    fn self_closing_void_element_just_drops_the_slash() {
        ...
        assert_eq!(
            expand_self_closing_tags(r#"<img src="a.png" />"#),
            r#"<img src="a.png">"#
        );
    }
    ...
    fn table_context_wrapper_wraps_td_and_th() {
        assert_eq!(
            table_context_wrapper("td"),
            ("<table><tbody><tr>", "</tr></tbody></table>", 3)
        );
        assert_eq!(
            table_context_wrapper("TH"),
            ("<table><tbody><tr>", "</tr></tbody></table>", 3)
        );
    }
