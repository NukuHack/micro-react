...
fn strips_named_import_and_records_specifier() {
	...
	assert_eq!(specifiers[0].named, vec!["useState".to_string()]);
...
fn strips_default_import_and_records_specifier() {
	...
	assert_eq!(specifiers[0].named, Vec::<String>::new());
}
...
fn strips_named_import_with_multiple_bindings_and_extra_whitespace() {
	...
	assert_eq!(code, "");
	assert_eq!(specifiers[0].named, vec!["a".to_string(), "b".to_string(), "c".to_string()]);
}
...
fn import_line_with_trailing_code_on_same_line_is_left_untouched() {
	// Not a whole-line import (regex requires the whole line to match), so
	// it must be left alone rather than partially rewritten.
...
fn only_first_export_default_is_touched() {
	// Mirrors the original non-global regexes: only the first match in the
	// whole source is rewritten.
...
#[test]
fn named_reexport_alias_with_irregular_whitespace_around_as() {
	let out = rewrite_exports_str("const a = 1;\nexport { a  as   c };");
	assert_eq!(out, "const a = 1;\n\nexports.c = a;");
}
...
fn identifier_containing_as_is_not_mistaken_for_the_as_keyword() {
	// `gas` contains the substring "as" but isn't the `as` keyword.
...
// ─── realistic combined file, mirroring hello.jsx's shape ───
...
fn realistic_module_with_default_and_named_exports() {
	...
	// None of these should survive into the executable body.
