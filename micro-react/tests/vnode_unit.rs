...
use micro_react::vnode::{
    next_id, ComponentFn, Children, NodeRef, PropVal, Props, Template, VNode, VNodeInner,
};
...
fn element_builder_child_and_children_accumulate() {
    let v = VNode::tag("ul")
        ...
        .children(vec![
            VNode::tag("li").text("two").build(),
            VNode::tag("li").text("three").build(),
        ])
        .build();
    match v.inner {
