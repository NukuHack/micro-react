...
fn substitutes_multiple_text_holes_in_one_run() {
    let vn = tpl(
        &["<p>", " of ", "</p>"],
        vec![JsValue::from_f64(3.0), JsValue::from_f64(10.0)],
    );
    let (container, _root) = mount(vn);
...
fn capture_suffix_survives_html_lowercasing_and_fires_first() {
    ...
    let vn = tpl(
        &["<div onClickCapture=", "><button onClick=", ">Click</button></div>"],
        vec![capture_fn.into(), bubble_fn.into()],
    );
    let (container, _root) = mount(vn);
    let btn = container.query_selector("button").unwrap().unwrap();
    click_on(&btn);
...
fn component_hole_receives_props_and_children() {
    ...
    let vn = tpl(
        &["<", " name=", ">hello</", ">"],
        vec![comp_fn.clone().into(), JsValue::from_str("Greeter"), comp_fn.into()],
    );
    let (container, _root) = mount(vn);
...
fn keyed_list_reorder_preserves_dom_identity_through_html_template() {
    ...
    let texts: Vec<String> = (0..lis_after.length())
        .map(|i| lis_after.get(i).unwrap().unchecked_into::<web_sys::Element>().text_content().unwrap())
        .collect();
...
fn builds_a_medium_sized_page_end_to_end() {
    ...
    let banner = if show_banner {
        tpl(&["<div className=\"banner\">3 items</div>"], vec![])
    } else {
        JsValue::NULL
    };
    ...
    let page = tpl(
        ...
        vec![
            JsValue::from_str("My Todos"),
            banner,
            items.into(),
            JsValue::from_str("(c) micro-react"),
        ],
    );
    ...
    assert_eq!(
        container.query_selector("header h1").unwrap().unwrap().text_content().unwrap(),
        "My Todos"
    );
    ...
    assert_eq!(
        container.query_selector("footer").unwrap().unwrap().text_content().unwrap(),
        "(c) micro-react"
    );
...
fn multiple_sibling_trs_as_template_roots_all_survive() {
    let vn = tpl(
        ...
        vec![JsValue::from_str("one"), JsValue::from_str("two")],
    );
    let (container, _root) = mount(vn);
...
fn gradient_units_survives_html_lowercasing() {
    let vn = tpl(
        &["<svg><linearGradient gradientUnits=\"", "\"></linearGradient></svg>"],
        vec![JsValue::from_str("userSpaceOnUse")],
    );
    let (container, _root) = mount(vn);
...
fn preserve_aspect_ratio_survives_html_lowercasing() {
    let vn = tpl(
        ...
        vec![JsValue::from_str("xMidYMid meet")],
    );
    let (container, _root) = mount(vn);
    let svg = container.query_selector("svg").unwrap().unwrap();
