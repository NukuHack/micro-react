...
fn use_effect_runs_after_mount() {
    ...
    let comp = ComponentFn::infallible(move |_props: Props| {
        ...
        use_effect_nodrop(move || { *ran.borrow_mut() = true; }, Some(vec![]));
        VNode::text("hi")
    });
...
fn use_effect_cleanup_runs_on_unmount() {
    ...
    let comp = ComponentFn::infallible(move |_props: Props| {
        ...
        use_effect(move || {
            ...
            Box::new(move || { *cleaned.borrow_mut() = true; }) as Box<dyn FnOnce()>
        }, Some(vec![]));
        VNode::text("hi")
    });
...
fn use_effect_does_not_rerun_when_deps_are_unchanged() {
    ...
    let comp = ComponentFn::infallible(move |_props: Props| {
        ...
        *setter_slot_for_comp.borrow_mut() = Some(set_tick);
        ...
        use_effect_nodrop(move || { *run_count.borrow_mut() += 1; }, Some(vec![]));
        VNode::text("hi")
    });
    ...
    let comp = ComponentFn::infallible(move |_props: Props| {
        ...
        *setter_slot_for_comp.borrow_mut() = Some(set_tick);
        ...
        let _memoized = use_memo(
            move || { *compute_count.borrow_mut() += 1; dep * 2 },
...
fn use_id_is_unique_per_component_and_stable_format() {
    ...
    let make_comp = || ComponentFn::infallible(|_props: Props| {
        let id = use_id();
        VNode::text(id)
    });
